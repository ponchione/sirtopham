...
var newLLMManager = func() llmManager {
...
func newLLMCmd(configPath *string) *cobra.Command {
	...
	cmd.AddCommand(newLLMStatusCmd(configPath), newLLMUpCmd(configPath), newLLMDownCmd(configPath), newLLMLogsCmd(configPath))
	return cmd
}

func newLLMStatusCmd(configPath *string) *cobra.Command {
	var jsonOutput bool
	cmd := &cobra.Command{
		Use:   "status",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return printLLMStatus(cmd.OutOrStdout(), status, jsonOutput)
		},
	}
...
func newLLMUpCmd(configPath *string) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			status, err := newLLMManager().EnsureUp(cmd.Context(), cfg)
			if err != nil {
				_ = printLLMStatus(cmd.ErrOrStderr(), status, false)
				return err
			}
			return printLLMStatus(cmd.OutOrStdout(), status, false)
		},
	}
}

func newLLMDownCmd(configPath *string) *cobra.Command {
	return &cobra.Command{
		Use:   "down",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := newLLMManager().Down(cmd.Context(), cfg); err != nil {
				return err
			}
...
func newLLMLogsCmd(configPath *string) *cobra.Command {
	var tail int
	cmd := &cobra.Command{
		Use:   "logs",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			logs, err := newLLMManager().Logs(cmd.Context(), cfg, tail)
			if err != nil {
				return err
			}
