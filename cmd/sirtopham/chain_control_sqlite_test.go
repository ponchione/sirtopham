...
func TestFinalizeRequestedChainStatusLogsTerminalPauseEvent(t *testing.T) {
...
func TestCloseErroredChainExecutionMarksFailedAndClearsActiveExecution(t *testing.T) {
	ctx := context.Background()
	store := chain.NewStore(newChainControlTestDB(t))
	chainID, err := store.StartChain(ctx, chain.ChainSpec{MaxSteps: 5, MaxResolverLoops: 1, MaxDuration: time.Hour, TokenBudget: 100})
	if err != nil {
	...
	if err := closeErroredChainExecution(ctx, store, chainID, "boom"); err != nil {
		t.Fatalf("closeErroredChainExecution returned error: %v", err)
	}
...
func newChainControlTestDB(t *testing.T) *sql.DB {
	t.Helper()
	ctx := context.Background()
	path := filepath.Join(t.TempDir(), "chain-control.db")
